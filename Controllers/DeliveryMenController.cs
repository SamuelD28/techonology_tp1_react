...
namespace technology_tp1.Controllers
{
    ...
    public class DeliveryMenController : CrudController<DeliveryMan>
    {
        ...
        {
            return Authenticate.Apply(HttpContext, "Administrator", () => base.GetAllRecord(start, end));
        }

        [HttpGet("{id}")]
        public IActionResult Get(int id)
        {
            return Authenticate.Apply(HttpContext, "Administrator", () => base.GetRecordById(id));
        }

        [HttpPost]
        public IActionResult Post([FromBody] DeliveryMan deliveryMan)
        {
            return Authenticate.Apply(HttpContext, "Administrator", () => base.CreateRecord(deliveryMan));
        }

        [HttpPut("{id}")]
        public IActionResult Update(int id, [FromBody] DeliveryMan deliveryMan)
        {
            return Authenticate.Apply(HttpContext, "Administrator", () => base.UpdateRecord(id, deliveryMan));
        }
